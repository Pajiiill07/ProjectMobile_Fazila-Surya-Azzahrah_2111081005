...
@section('content')
            ...
            <div class="card">
                        ...
                        <td class="sorting_1">
                            <div class="d-flex justify-content-start align-items-center user-name">
                                <div class="d-flex flex-column">
                                        <span class="fw-medium">{{$item->nama_department}}</span>
                                </div>
                            </div>
                        </td>
                        <td>
                            <span class="user-email">{{$item->deskripsi}}</span>
                        </td>
                        @can('isAdmin')
                        <td>
                            <div class="d-inline-block text-nowrap">
                                <button class="btn btn-sm btn-icon edit-record" data-id="{{$item->department_id}}" data-bs-toggle="offcanvas" data-bs-target="#offcanvasEditUser">
                                    <i class="bx bx-edit"></i> EDIT
                                </button>
                                <form action="department-backend/{{$item->department_id }}" method="post">
                                  @method('DELETE')
                                  @csrf
                                  <button class="btn btn-sm btn-icon delete-record" data-id="{{$item->department_id}}" onclick="return confirm('Yakin akan menghapus data ?')">
              ...
              @include('content.department.create')
              @include('content.department.update')
            </div> 
