            ...
            <div class="card">
              <div class="card-datatable table-responsive">
                <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                      ...
                      <div class="dt-buttons"> 
                        ...
                        <button class="dt-button buttons-collection dropdown-toggle btn btn-label-secondary mx-3" tabindex="0" aria-controls="DataTables_Table_0" type="button" aria-haspopup="dialog" aria-expanded="false">
                          ...
                          {{-- <button class="dt-button add-new btn btn-primary" tabindex="0" aria-controls="DataTables_Table_0" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddUser">
                            ...
                            <span class="d-none d-sm-inline-block">Add New Laporan</span>
                            </span>
                          </button>  --}}
                          @endcan
                        </div>
                  ...
                  <tbody>
                    @foreach ($lprns as $item)
                    <tr class="odd">
                      <td class="  control" tabindex="0" style="display: none;"></td>
                      <td><span>{{$item->laporan_id}}</span></td>
                      <td>
                      ...
                      <td>
                        <span>{{$item->tanggal_laporan}}</span>
                      </td>
                      <td>
                        <span>{{$item->tanggal_submit}}</span>
                      </td>
                      <td>
                        <span>{{$item->judul_laporan}}</span>
                      </td>
                      <td>
                        <span class="fw-medium">{{$item->isi_laporan}}</span>
                      </td>
                      @can('isAdmin')
                      <td>
                        <div class="d-inline-block text-nowrap">
                          {{-- <button class="btn btn-sm btn-icon edit-record" data-id="{{$item->laporan_id}}" data-bs-toggle=" offcanvas" data-bs-target="#offcanvasEditUser">
                            <i class="bx bx-edit"></i>
                          </button> --}}
                          <form action="laporan-backend/{{$item->laporan_id }}" method="post">
                            @method('DELETE')
                            @csrf
                            <button class="btn btn-sm btn-icon delete-record" data-id="{{$item->laporan_id}}" onclick="return confirm('Yakin akan menghapus data ?')">
              ...
              <!-- Offcanvas to add new laporan -->
              {{-- @include('content.laporan.create')
              @include('content.laporan.update') --}}
            </div>        
@endsection